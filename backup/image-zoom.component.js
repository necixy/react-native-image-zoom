  ...
  function ImageViewer() {
    ...
    _this.imagePanResponder = react_native_1.PanResponder.create({
      ...
      onPanResponderMove: function (evt, gestureState) {
        if (_this.isDoubleClick) {
          // 有时双击会被当做位移，这里屏蔽掉
          // Sometimes double-clicking will be regarded as displacement, shielded here
          return;
        }
        if (evt.nativeEvent.changedTouches.length <= 1) {
          // x 位移
          ...
          if (Math.abs(_this.horizontalWholeCounter) > 5 || Math.abs(_this.verticalWholeCounter) > 5) {
            // 如果位移超出手指范围，取消长按监听
            // If the displacement exceeds the finger range, cancel the long-press monitoring
            clearTimeout(_this.longPressTimeout);
          }
          if (_this.props.panToMove) {
            // 处理左右滑，如果正在 swipeDown，左右滑失效
            // Handling left and right sliding, if it is swipeDown, left and right sliding invalid
            if (_this.swipeDownOffset === 0) {
              ...
              if (true) {
                ...
                if (_this.horizontalWholeOuterCounter > 0) {
                  ...
                  // 溢出偏移量为0，正常移动
                  // The overflow offset is 0, normal movement
                }
                // 产生位移
                ...
                var horizontalMax = _this.props.imageWidth / 2;
            ...
            // If the height of the picture is greater than the height of the box, you can drag it vertically

            // #Nec
            if (true) {
              // #Nec
              var vMax = _this.props.imageHeight / 2;
              _this.positionY += diffY / _this.scale;

              if (_this.positionY < -vMax) {
                _this.positionY = -vMax;
              } else if (_this.positionY > vMax) {
                _this.positionY = vMax;
              }
    ...
    _this.panResponderReleaseResolve = function () {
      ...
      if (_this.props.enableCenterFocus && _this.scale < 1) {
        // 如果缩放小于1，强制重置为 1
        // If the zoom is less than 1, forcibly reset to
        _this.scale = 1;
        react_native_1.Animated.timing(_this.animatedScale, {
          toValue: _this.scale,
          duration: 100,
      ...
      if (_this.props.imageHeight * _this.scale <= _this.props.cropHeight) {
        // 如果图片高度小于盒子高度，纵向位置重置
        // If the height of the picture is smaller than the height of the box, the vertical position is reset
        // #Nec
        // _this.positionY = 0;
        // react_native_1.Animated.timing(_this.animatedPositionY, {
        //   toValue: _this.positionY,
      ...
      // If the height of the picture is greater than the height of the box, no black borders can appear vertically

      if (_this.props.imageHeight * _this.scale > _this.props.cropHeight) {
        // 纵向能容忍的绝对值
        // Absolute value tolerated vertically
        // #Nec
        // var verticalMax = (_this.props.imageHeight * _this.scale - _this.props.cropHeight) / 2 / _this.scale;
        var verticalMax = _this.props.imageHeight / 2;
      ...
      if (_this.props.imageWidth * _this.scale > _this.props.cropWidth) {
        // 纵向能容忍的绝对值
        // Absolute value tolerated longitudinally
        // #Nec
        // var horizontalMax = (_this.props.imageWidth * _this.scale - _this.props.cropWidth) / 2 / _this.scale;
        var horizontalMax = _this.props.imageWidth / 2;
        if (_this.positionX < -horizontalMax) {
      ...
      // After the dragging ends normally, if there is no zoom, directly return to 0,0 point
      if (_this.props.enableCenterFocus && _this.scale === 1) {
        // Wanted to disable this.
        // _this.positionX = 0;
        // _this.positionY = 0;
        // react_native_1.Animated.timing(_this.animatedPositionX, {
        //   toValue: _this.positionX,
